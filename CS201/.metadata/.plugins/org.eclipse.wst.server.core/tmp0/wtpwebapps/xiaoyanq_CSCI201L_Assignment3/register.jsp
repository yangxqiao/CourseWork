...
<head>
	...
	<style type="text/css">
		...
		#username, #password, #passwordcopy{
			width: 100%;
			height: 30px;
		}
	...
	<script>
		...
		function val(e){
			var xhttp = new XMLHttpRequest();
			xhttp.open("GET", "register_servlet?username=" + document.registerform.username.value +
				"&password=" + document.registerform.password.value + 
				"&passwordcopy=" + document.registerform.passwordcopy.value, false);
				xhttp.send();
				if (xhttp.responseText.trim().length > 0) {
					...
					window.localStorage.setItem('username', document.registerform.username.value);
					return true;
				}
		}
...
<body>
	...
	<div id="content">
		<form name="registerform" method="GET" action="HomePage.jsp" onsubmit="return val(event);">
			...
			<div class="info">
				 ...
				 <input id="passwordcopy" type="text" name="passwordcopy" /><br />
			</div>
