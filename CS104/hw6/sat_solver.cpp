...
size_t check_correct_clause(map<int, int>& sol, map<int, set<int> >& all_clauses);
bool solveHelper(map<int, int>& sol, map<int, set<int> >& all_clauses, size_t curr_var);
bool isValid(map<int, int>& sol, map<int, set<int> >& all_clauses, size_t curr_var);
void printSolution(map<int, int>& sol, ofstream& ofile);
...
size_t check_correct_clause(map<int, int>& sol, map<int, set<int> >& all_clauses)
{
	...
	for(it = all_clauses.begin(); it != all_clauses.end(); ++it)
	{
		...
		{
			int var = abs(*jt);
			map<int, int>::iterator ft = sol.find(var);
			int val = ft->second;
...
bool isValid(map<int, int>& sol, map<int, set<int> >& all_clauses, size_t curr_var)
{
	...
	for(pt = all_clauses.begin(); pt != all_clauses.end(); ++pt)
	{
		...
		{
			int var = abs(*it);
			map<int, int>::iterator ht = sol.find(var);
...
void printSolution(map<int, int>& sol, ofstream& ofile)
{
	for(unsigned int i = 1; i < sol.size()+1; i++)
	{
		map<int, int>::iterator it = sol.find(i);
		if(it->second != -1)
		{
			ofile << i << " = " << it->second << endl;
		}
	}
}